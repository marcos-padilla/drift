"""
System prompt builder for the Drift framework.

This module provides a class-based system for building comprehensive system
prompts for AI code assistants, including identity, environment, tools,
security guidelines, and operational instructions.
"""
...
class PromptBuilder:
    """
    Builder for constructing system prompts for AI code assistants.

    This class provides methods to build comprehensive system prompts with
    various sections including identity, environment, tools, security,
    and operational guidelines.

    Parameters
    ----------
    config : Configuration
        Configuration object containing settings and instructions.
    ...
    >>> prompt = builder.build(user_memory="User prefers Python 3.12")
    """
    ...
    def build(
        ...
        -------
        str
            Complete system prompt as a formatted string.

        Examples
        --------
        >>> builder = PromptBuilder(config)
        >>> prompt = builder.build(
        ...     user_memory="User prefers TypeScript",
        ...
        parts.append(self._build_environment_section())

        # Optional sections
        if tools:
            parts.append(self._build_tool_guidelines_section(tools))

        parts.append(self._build_agents_md_section())
        parts.append(self._build_security_section())

        if self.config.developer_instructions:
            parts.append(
                self._build_developer_instructions_section(
        ...
        if self.config.user_instructions:
            parts.append(
                self._build_user_instructions_section(self.config.user_instructions),
            )

        if user_memory:
            parts.append(self._build_memory_section(user_memory))

        parts.append(self._build_operational_section())
...
The user has granted you access to run tools in service of their request. Use them when needed."""
    ...
    def _get_shell_info(self) -> str:
        """
        Get shell information based on the current platform.

        Returns
        -------
        str
            Shell information string.
        """
    ...
    def _build_tool_guidelines_section(self, tools: list[dict[str, Any]]) -> str:
        """
        Build the tool guidelines section of the prompt.
        ...
        -------
        str
            Tool guidelines section content.
        """
        regular_tools: list[dict[str, Any]] = [
            t for t in tools if not t.get("name", "").startswith("subagent_")
        ]
        subagent_tools: list[dict[str, Any]] = [
            t for t in tools if t.get("name", "").startswith("subagent_")
        ]

        guidelines: str = """# Tool Usage Guidelines
...
"""
        ...
        for tool in regular_tools:
            ...
            guidelines += f"- **{tool.get('name', 'unknown')}**: {description}\n"
...
6. **Sub-Agents**:
   ...
   - Use sub-agents when the task involves complex refactoring, codebase exploration, or system-wide analysis"""
    ...
    def _build_agents_md_section(self) -> str:
        """
        Build the AGENTS.md specification section.

        Returns
        -------
        str
            AGENTS.md specification section content.
        """
...
- The contents of the AGENTS.md file at the root of the repo and any directories from the CWD up to the root are included with the developer message and don't need to be re-read. When working in a subdirectory of CWD, or a directory outside the CWD, check for any AGENTS.md files that may be applicable."""
    ...
    def _build_security_section(self) -> str:
        """
        Build the security guidelines section.

        Returns
        -------
        str
            Security guidelines section content.
        """
...
6. **Security First**: Always apply security best practices. Never introduce code that exposes, logs, or commits secrets, API keys, or other sensitive information."""
    ...
    def _build_operational_section(self) -> str:
        """
        ...
        -------
        str
            Operational guidelines section content.
        """
...
- Do not use one-letter variable names unless explicitly requested."""
    ...
    def _build_developer_instructions_section(self, instructions: str) -> str:
        """
        ...
        -------
        str
            Developer instructions section content.
        """
...
Follow these instructions carefully as they contain important context about this specific project."""
    ...
    def _build_user_instructions_section(self, instructions: str) -> str:
        """
        ...
        -------
        str
            User instructions section content.
        """
...
{instructions}"""
    ...
    def _build_memory_section(self, memory: str) -> str:
        """
        Build the user memory section.

        Parameters
        ----------
        memory : str
            User memory/context from previous interactions.
...
def get_compression_prompt() -> str:
    """
    Get the prompt template for compressing conversation context.
...
def create_loop_breaker_prompt(loop_description: str) -> str:
    """
    Create a prompt to break out of repetitive loops.

    Parameters
    ----------
    loop_description : str
        Description of the detected loop pattern.
